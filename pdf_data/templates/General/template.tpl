...
$footer = '<div style="background: #4676bd; font-family: Arial; font-size: 12px; height: 28px; font-weight: bold; text-align: center;margin-left:-22px;">
...
<?php if( $product_count > 0 ){ ?>
    <?php while ( $loop->have_posts() ){ ?>
        <?php $loop->the_post(); ?>
		<?php if(get_post_meta($loop->post->ID, "_product_attributes", true) == '') { ?>
        <?php $product = new WC_Product($loop->post->ID); ?>
            <div class="product">
                <tocentry name="" content="<?php echo $post->post_title; ?>" />
        		<div class="image">
                    <?php if (has_post_thumbnail( $loop->post->ID )){ ?>
                		<?php echo get_the_post_thumbnail($loop->post->ID, array(196,196)); ?>
                	<?php }else{ ?>
                		<img src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/placeholder.png" alt="" style="max-width: 196px; max-height: 196px;" />
                    <?php } ?>
        		</div>
                <div class="right_blick">
                ...
                <div class="name">
                    <?php echo strip_tags(self::utf8_substr(html_entity_decode($post->post_title, ENT_QUOTES, 'UTF-8'), 0, 40)); ?>
                </div>
                <div class="model"><?php echo __('SKU:').' '.get_post_meta($loop->post->ID, '_sku', true); ?></div>
                <div class="description">
                    <?php echo strip_tags(self::utf8_substr(html_entity_decode($post->post_content, ENT_QUOTES, 'UTF-8'), 0, 200)); ?>
                </div>
            </div>
	<?php }else{ ?>
        <?php $product = new WC_Product_Variation ($loop->post->ID); ?>
            <div class="product">
                <tocentry name="" content="<?php echo $post->post_title; ?>" />
        		<div class="image">
                    <?php if (has_post_thumbnail( $loop->post->ID )){ ?>
                ...
                <div class="right_blick">
                        ...
                        <div class="price">
						                            <div>
							<?php echo 'A partire da ' . str_replace(".", ",", get_post_meta( $loop->post->ID, "_price", true)); ?>€
							</div>

							<div>
														<?php
$meta = get_post_meta( $loop->post->ID, "_product_attributes", true );
